...
from .model import add_timeseries_reference, get_wml_data, get_refts
...
class d(dict):
    """
    Method for preventing dict key errors.

    Whenever a dictionary key is referenced but doesn't exist, returns an empty string
...
def get_app_workspace():
    ...
    The app workspace is used to store session file data before being uploaded
    to HydroShare.
    """
...
def process_form_data(form):
    """
    Converts a POST form to a REFTS object.

    This function is designed to convert forms generated by the CUAHSI HydroClient
    to properly formatted REFTS objects that can be added to a session.
    """
...
def add_refts_to_session(refts, session_id):
    """
    Adds a REFTS object to a session.

    This function can take a properly formatted REFTS object from any source
    ...
    return timeseries_id_list


def get_refts_from_hydroshare(resource_id, aggregation_path):
    """
    Gets REFTS data from HydroShare.

    This function will extract REFTS data from a HydroShare resource or 
    aggregation. Reference Time Series aggregations will be downloaded and parsed
    directly. Time Series data will be obtained from HydroServer if one has been
    set up.
    """

    if resource_id:
        request_url = f"{hydroshare_url}/hsapi/resource/{resource_id}/files/"
        response = requests.get(request_url)
        refts_list = []
        if response.status_code == 200:
            hydroshare_files = json.loads(response.content)["results"]
            for hydroshare_file in hydroshare_files:
                if hydroshare_file["logical_file_type"] == "RefTimeseriesLogicalFile":
                    if aggregation_path == None or aggregation_path == hydroshare_file["url"]:
                        request_url = hydroshare_file["url"]
                        response = requests.get(request_url)
                        refts_list.append(json.loads(response.content))
        if aggregation_path == None:
            request_url = f"{hydroserver_url}/refts/catalog/?network_id={resource_id}"
        else:
            request_url = f"{hydroserver_url}/refts/catalog/?network_id={resource_id}&database_id={aggregation_path}"
        response = requests.get(request_url)
        refts = json.loads(response.content)
        if refts:
...
def download_soap_wml(refts_list):
...
def download_rest_wml(refts_list):
    async def fetch_all(refts_list):
        tasks = []
        async with ClientSession() as session:
            for refts in refts_list:
...
def extract_soap_wml(soap_data, wml_version, unzip):
...
def extract_rest_wml(rest_data, unzip):

    if unzip:
        try:
            extracted_data = zipfile.ZipFile(io.BytesIO(rest_data), "r")
            for finfo in extracted_data.infolist():
                ifile = extracted_data.open(finfo)
                line_list = ifile.readlines()
                rest_data = b''.join(line_list)
        except:
            rest_data = rest_data
...
def create_refts_file(session_id, workspace, refts_metadata):

    refts_data = get_refts(session_id)
    print(refts_data)
    raise Exception
    refts_filepath = "/".join((workspace, session_id, f"{refts_metadata['res_filename']}.refts.json",))
